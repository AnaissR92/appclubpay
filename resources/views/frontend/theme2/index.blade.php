        ...
        <div class="pb-6 pt-6 md:pb-32 grid sm:grid-cols-2 md:grid-cols-3 gap-5 xl:gap-8" id="categories">
            @foreach ($food_categories ?? [] as $category)
                <div class="bg-white dark:bg-secondary/50 rounded-xl shadow-shadowitem hover:shadow-shadowdark transition">
                    <a href="{{ route('restaurant.menu.item', ['restaurant' => $restaurant->slug, 'food_category' => $category->id] + $append) }}"><img src="{{ $category->category_image_url }}" alt=""
                            class="w-full rounded-t-xl h-56 object-cover" onerror="this.src='{{ $default_category_image }}'" /></a>
                    <div class="p-4">
                        <a href="{{ route('restaurant.menu.item', ['restaurant' => $restaurant->slug, 'food_category' => $category->id] + $append) }}" class="font-bold dark:text-white">{{ $category->local_lang_name }}</a>
                    </div>
                </div>
            @endforeach
        </div>
